package hu.ponte.hr;

import hu.ponte.hr.services.SignService;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;
import org.springframework.util.ResourceUtils;

import java.io.FileNotFoundException;
import java.io.IOException;
import java.nio.file.Files;
import java.util.LinkedHashMap;
import java.util.Map;

import static org.hamcrest.MatcherAssert.assertThat;
import static org.hamcrest.Matchers.equalTo;
import static org.hamcrest.core.Is.is;

/**
 * @author zoltan
 */
@RunWith(SpringRunner.class)
@SpringBootTest()
public class SignTest {
    Map<String, String> files = new LinkedHashMap<>() {
        {
            put("cat.jpg", "");
            put("enhanced-buzz.jpg", "");
            put("rnd.jpg", "");
        }
    };

    @Autowired
    SignService signService;

    @Test
    public void test_01() throws IOException {
        byte[] data = Files.readAllBytes(ResourceUtils.getFile(ResourceUtils.CLASSPATH_URL_PREFIX + "images/cat.jpg").toPath());
        String result = signService.sign(data);
        assertThat(result, is(equalTo(files.get("cat.jpg"))));
    }

    @Test
    public void test_02() throws IOException {
        byte[] data = Files.readAllBytes(ResourceUtils.getFile(ResourceUtils.CLASSPATH_URL_PREFIX + "images/enhanced-buzz.jpg").toPath());
        String result = signService.sign(data);
        assertThat(result, is(equalTo(files.get("enhanced-buzz.jpg"))));
    }

    @Test
    public void test_03() throws IOException {
        byte[] data = Files.readAllBytes(ResourceUtils.getFile(ResourceUtils.CLASSPATH_URL_PREFIX + "images/rnd.jpg").toPath());
        String result = signService.sign(data);
        assertThat(result, is(equalTo(files.get("rnd.jpg"))));
    }


}
